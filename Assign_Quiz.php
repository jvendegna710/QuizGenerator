    ...
    if((empty($_POST['quizID'])) || (empty($_POST['numNames']))){
        ...
        foreach($_POST['quizID'] as $qid){     
            for($i=0; $i < $nNum; $i++){
                ...
                //echo "NAME IS ".$name;
                $sql = "SELECT USERNAME FROM USERLIST WHERE ID =".$name;
                ...
                $sql = "INSERT INTO QUIZ_USERS (USERID,QUIZID,USERNAME,DATE_ASSIGNED) VALUES (:1,:2,:3,SYSDATE)";
                $results = $db->ExecSQL($sql,array($name,$qid,$uName));
                if(!$results){
        ...
        if (!empty($error)){
            ...
            <div id='formdiv'>
                ...
                <div id='qtable' style='height:75%;!important'>
                    <table class='table table-bordered table-striped table-condensed table-hoverable' style='width:100%;'>
                    ";
                foreach($_POST['quizID'] as $qid){
                ...
                <div style='margin-top:.5em; margin-left:70%;'><a href='../Quiz/QUIZ.php' class='btn btn-primary'>Continue to Quiz</a></div>
                </div><!-- END formdiv--> 
                ";
            }//Ends IF $inst
        }
...
function Confirm(){
    ...
    $idlist = '';
    foreach($_POST['quizID'] as $value){
        $idlist .= '<input type="hidden" name="quizID[]" value="'. $value. '">'."\n";
    }

    $nNum = $_POST['numNames'];
    $qName = '';
    $sql = "SELECT QUIZ_NAME FROM QUIZ_MASTER WHERE ";
    $qCount = count($_POST['quizID']);   

    if($qCount > 1){
        $place = 1;     
        foreach($_POST['quizID'] as $q){            
            if($place > 1){              
                $sql .= " OR QID=".$q;
            }else{
                $sql .= " QID=".$q;
...
<form method='POST' action=''>
    <div id='formdiv' style='height:45em'>
        <div id='header'>Choose Whom The Quiz shall be assigned To:</div>            
            <div style='margin:0 auto .5em auto; width:80%; '>Department:                    
                <select id='deptSelect'  class='ddown'>
                    ...
                    <option value=\"= 'CLERICAL - JUDGMENT'\">CLERICAL-JUDGMENT</option>
            ...
            <table class='table table-bordered table-striped table-condensed table-hoverable'  id='nameTable'></table>
            </div><br>             
        ...
        <div style='display:inline; width:30%; '>
            ...
            <a href='Assign_Quiz.php' class='btn btn-warning' >Cancel</a>
        </div><!-- END bottomdiv-->            
    </div><!-- END FORMDIV      --> 
</form><!-- END form-->   
";
            ...
            <table class='table table-striped' style='padding-top:1.5em; width:100%;'>
                <tr class='c_row'>
                    ...
                    <tr class='c_row'>
                        ...
                        <td style='text-align:center'>
                            <select id='quizNameDD' name='quizID[]' class='ddown' multiple>                                                          
                            </select>
                        </td>
                    </tr>           
                </table>
